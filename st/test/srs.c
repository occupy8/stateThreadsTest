...
{
    srs_trace("thread function in hahaha");
    listen_test();
...
int listen_test()
{
    ...
    srs_trace("create linux socket ..... port=%d, fd=%d", port, _fd);
    
    if ((_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
        ret = -1;
        srs_trace("create linux socket error. port=%d, ret=%d", port, ret);
        return ret;
    }
    srs_trace("create linux socket success. port=%d, fd=%d", port, _fd);
    
    int reuse_socket = 1;
    if (setsockopt(_fd, SOL_SOCKET, SO_REUSEADDR, &reuse_socket, sizeof(int)) == -1) {
        ret = -1;
        srs_trace("setsockopt reuse-addr error. port=%d, ret=%d", port, ret);
        return ret;
    }
    srs_trace("setsockopt reuse-addr success. port=%d, fd=%d", port, _fd);

    struct sockaddr_in addr;
    addr.sin_family = AF_INET;
    addr.sin_port = htons(port);
    addr.sin_addr.s_addr = inet_addr(ip);
    if (bind(_fd, (const struct sockaddr*)&addr, sizeof(struct sockaddr_in)) == -1) {
        ret = -1;
        srs_trace("bind socket error. ep=%s:%d, ret=%d", ip, port, ret);
        return ret;
    }
    srs_trace("bind socket success. ep=%s:%d, fd=%d", ip, port, _fd);
    
    if (listen(_fd, 100) == -1) {
        ret = -1;
        srs_trace("listen socket error. ep=%s:%d, ret=%d", ip, port, ret);
        return ret;
    }
    srs_trace("listen socket success. ep=%s:%d, fd=%d", ip, port, _fd);
    st_netfd_t _stfd;

    if ((_stfd = st_netfd_open_socket(_fd)) == NULL){
        ret = -1;
        srs_trace("st_netfd_open_socket open socket failed. ep=%s:%d, ret=%d", ip, port, ret);
        return ret;
    }
    srs_trace("st open socket success. ep=%s:%d, fd=%d", ip, port, _fd);
    ...
    while(1){
	    ...
	    if(client_stfd == NULL){
		    // ignore error.
		    if (errno != EINTR) {
			    srs_trace("ignore accept thread stoppped for accept client error");
		    }
		    return ret;
	    }
	    srs_trace("get a client. fd=%d", st_netfd_fileno(client_stfd));
    }    
